...
function renderPortfolio(){
  const result=LittleCore.ledger(data),base=result.positions.filter(shown),ps=typeof QuoteUI!=='undefined'?QuoteUI.positions(base):base,open=ps.filter(p=>p.quantity>0),sales=result.sales.filter(shown);
  $('#stats').innerHTML=stat('มูลค่าคงเหลือ',money(open.reduce((s,p)=>s+p.market,0)),currency,open.some(p=>p.quote)?'ผสมราคาออนไลน์ / กรอกเอง · ดูเวลาในรายการ':'ตามราคากรอกเอง / รายการล่าสุด','◈')+stat('ต้นทุนคงเหลือ',money(open.reduce((s,p)=>s+p.basis,0)),currency,'รวมค่าธรรมเนียมซื้อ','▤')+stat('กำไรที่ยังไม่ขาย',money(open.reduce((s,p)=>s+p.unrealized,0)),currency,'ไม่รวมค่าขายในอนาคต','↗')+stat('กำไรที่ขายแล้ว',money(sales.reduce((s,p)=>s+p.profit,0)),currency,'ทุกช่วงเวลา · หักค่าธรรมเนียม','✓');
  const rows=ps.map((p,i)=>`<tr><td><b>${esc(p.symbol)}</b><br><small>${meta(p)}</small></td><td>${qty(p.quantity)}</td><td>${money(p.cost)}</td><td>${money(p.current)}${p.quote?QuoteUI.details(p.quote):'<br><small>ราคากรอกเอง / รายการล่าสุด</small>'} <button class="row-action" data-price="${i}">กรอกราคาเอง</button></td><td>${signed(p.unrealized)}</td><td>${signed(p.realized)}</td><td>${p.quantity>0?`<button class="row-action" data-sell="${i}">ขาย</button>`:''}</td></tr>`);
  ...
  $('#feature-page').innerHTML=`<article class="panel"><h2>จำนวนคงเหลือและต้นทุนเฉลี่ย</h2><p>แยกต้นทุนตามพอร์ต สินทรัพย์ สกุลเงิน และกลยุทธ์ · ราคาประเมินเริ่มจากรายการล่าสุดจนกว่าคุณจะอัปเดต · ราคาออนไลน์ใช้ชั่วคราวเฉพาะหน้านี้ ปิดตัวเลือกเพื่อกลับไปใช้ราคากรอกเอง</p>${rows.length?gridTable(['สินทรัพย์ / พอร์ต','คงเหลือ','ต้นทุนเฉลี่ย / หน่วย','ราคาประเมิน / หน่วย','ยังไม่ขาย','ขายแล้ว',''],rows):'<p class="empty">ยังไม่มีรายการในตัวกรองนี้</p>'}</article><article class="panel"><h2>ประวัติซื้อ–ขาย</h2><p>รายการเดิมเป็นยอดยกมา ไม่ได้สร้างประวัติขายย้อนหลัง · ถ้าเวลาเท่ากัน ระบบใช้ลำดับที่บันทึก</p>${gridTable(['วันที่ / เวลา','รายการ','สินทรัพย์','จำนวน','ราคา / หน่วย','ค่าธรรมเนียม','กำไรขาย',''],events)}</article>`;
  $$('[data-price]').forEach(b=>b.onclick=()=>{if(typeof QuoteUI!=='undefined'){$('#quote-spot').checked=false;render()}openFeature('price',null,base[+b.dataset.price])});
