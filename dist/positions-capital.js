...
valid=function(d){return pcValid(d)&&d.trades.every(FuturesCore.valid)&&CapitalCore.validate(d)&&['trades','holdings','plans','reviews','spotTransactions','cashFlows','equitySnapshots'].reduce((n,k)=>n+(d[k]||[]).length,0)+d.trades.reduce((n,t)=>n+(t.closes||[]).length+(t.entries||[]).length+(t.charges||[]).length,0)<=10000};
...
function renderClosures(){
 ...
 $('#closures-history').innerHTML=t.closes?.length?gridTable(['วันปิด','จำนวน','ราคาออก','กำไรสุทธิ',''],FuturesCore.closeEvents(t).map(e=>`<tr><td>${esc(e.date)}</td><td>${qty(e.closedQuantity)}</td><td>${money(e.exit,t.asset==='BTCUSDT'?'USDT':'USD')}</td><td>${money(e.profit,t.currency)} ${t.currency}</td><td><button type="button" data-edit-close="${esc(e.eventId)}">แก้ไข</button> <button type="button" data-delete-close="${esc(e.eventId)}">ลบ</button></td></tr>`)):'<p>ยังไม่มีไม้ปิด</p>';
