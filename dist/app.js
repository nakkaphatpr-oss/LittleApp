...
function resetCloudUser(user){
  ...
  data={trades:[],holdings:[]};$('#editor').close();$('#account-dialog').close();render();
...
function render(){renderAccounts();const ts=data.trades.filter(t=>t.currency===currency&&accountMatches(t)&&tradeMatches(t)),closed=ts.filter(t=>t.exit!==null),hs=data.holdings.filter(h=>h.currency===currency&&accountMatches(h)&&strategyMatches(h));const total=closed.reduce((n,t)=>n+pnl(t),0),wins=closed.filter(t=>pnl(t)>0).length,cost=hs.reduce((n,h)=>n+h.quantity*h.cost+h.fee,0),value=hs.reduce((n,h)=>n+h.quantity*h.current,0);
