...
const KEY='littleapp-v1', currencies=['USD','USDT','THB'];
...
function render(){const ts=data.trades.filter(t=>t.currency===currency),closed=ts.filter(t=>t.exit!==null),hs=data.holdings.filter(h=>h.currency===currency);const total=closed.reduce((n,t)=>n+pnl(t),0),wins=closed.filter(t=>pnl(t)>0).length,cost=hs.reduce((n,h)=>n+h.quantity*h.cost+h.fee,0),value=hs.reduce((n,h)=>n+h.quantity*h.current,0);
 ...
 renderChart(closed);const groups={};hs.forEach(h=>groups[h.asset]=(groups[h.asset]||0)+h.quantity*h.cost+h.fee);$('#allocation').innerHTML=cost?Object.entries(groups).map(([k,n])=>`<div class="allocation-row"><div><span>${esc(k)}</span><b>${money(n/cost*100)}%</b></div><div class="track"><i style="width:${n/cost*100}%"></i></div></div>`).join(''):`<div class="empty-portfolio"><div class="empty-symbol">◈</div><h2>เริ่มสะสมอนาคตของคุณ</h2><p>ยังไม่มีสินทรัพย์ในสกุล ${currency}</p></div>`;
