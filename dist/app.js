...
const accountMatches=r=>(typeof LayerCore==='undefined'||LayerCore.matches(r,accounts(),$('#layer-filter')?.value||'all'))&&($('#account-filter').value==='all'||(r.accountId||'unassigned')===$('#account-filter').value);
function validAccounts(x){const list=x.accounts||defaultAccounts();return Array.isArray(list)&&list.length<=100&&list.every((a,i)=>a&&typeof a.id==='string'&&a.id.length>0&&a.id.length<=100&&a.id!=='all'&&a.id!=='unassigned'&&list.findIndex(b=>b.id===a.id)===i&&typeof a.broker==='string'&&a.broker.trim().length>0&&a.broker.length<=60&&typeof a.name==='string'&&a.name.trim().length>0&&a.name.length<=60&&(a.layer==null||['unclassified','wealth','investment','speculate','gambling'].includes(a.layer)))&&[...x.trades,...x.holdings].every(r=>r.accountId==null||r.accountId==='unassigned'||list.some(a=>a.id===r.accountId))}
