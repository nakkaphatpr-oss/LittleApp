...
$('#account-form').onsubmit=async e=>{e.preventDefault();if(!featureAllowed())return;$('#account-form').inert=true;try{const next=AccountsCore.edit({...data,accounts:accounts()},$('#account-picker').value,$('#account-name').value,$('#account-broker').value,()=>crypto.randomUUID(),$('#account-layer')?.value);if(!valid(next))throw Error('ข้อมูลพอร์ตไม่ถูกต้องหรือเกินขีดจำกัด 100 พอร์ต');if(await save(next)){$('#account-dialog').dataset.dirty='';$('#account-dialog').close();toast('บันทึกพอร์ตแล้ว')}}catch(err){$('#account-error').textContent=err.message}finally{$('#account-form').inert=false}};
